...
reader = easyocr.Reader(['en', 'id'], gpu=True)  # menambahkan 'id' untuk Bahasa Indonesia
...
def main():
    ...
    """
    # Coba akses USB webcam (biasanya index 1)
    cap = cv2.VideoCapture(1)

    # Jika tidak bisa, coba fallback ke webcam bawaan
    if not cap.isOpened():
        print("USB webcam tidak ditemukan, mencoba webcam bawaan...")
        cap = cv2.VideoCapture(0)

    # Jika tetap gagal, keluar dari program
    if not cap.isOpened():
        print("Tidak dapat membuka webcam. Pastikan USB webcam sudah terhubung.")
        return
    
    # Coba atur fokus kamera (jika didukung)
    cap.set(cv2.CAP_PROP_AUTOFOCUS, 0)  # Matikan autofocus jika ada
    cap.set(cv2.CAP_PROP_FOCUS, 50)  # Atur fokus manual (nilai bisa disesuaikan)
